...
def get_celebrities(
    ...
    """
    celebrity_service = CelebrityService(db)
    celebrities = celebrity_service.get_all_celebrities(skip=skip, limit=limit, search=search)
...
def get_popular_celebrities(
    limit: int = Query(10, ge=1, le=50, description="Number of popular celebrities to return"),
    db: Session = Depends(get_db)
):
    """
...
def get_celebrities_by_tag(
    ...
    """
    celebrity_service = CelebrityService(db)
    celebrities = celebrity_service.get_celebrities_by_tag(tag_name, skip=skip, limit=limit)
    return [CelebrityResponse.model_validate(celebrity) for celebrity in celebrities]
...
@router.delete("/{celebrity_id}/tags/{tag_name}", status_code=status.HTTP_204_NO_CONTENT)
