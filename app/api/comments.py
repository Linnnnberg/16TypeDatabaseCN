...
def get_comments(
    ...
    include_replies: bool = Query(True, description="Whether to include reply comments"),
    db: Session = Depends(get_db)
):
    """
...
def get_my_comments(
    ...
    """
    comment_service = CommentService(db)
    comments = comment_service.get_user_comments(current_user.id, skip=skip, limit=limit)
    return [CommentResponse.model_validate(comment) for comment in comments]
...
def get_celebrity_comment_statistics(
    celebrity_id: str,
    db: Session = Depends(get_db)
):
    """
...
def get_user_comment_statistics(
    user_id: str,
    db: Session = Depends(get_db)
):
    """
