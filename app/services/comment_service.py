...
class CommentService:
    ...
    def create_comment(self, user_id: str, comment_data: CommentCreate) -> Comment:
        ...
        celebrity = self.db.query(Celebrity).filter(Celebrity.id == comment_data.celebrity_id).first()
        if not celebrity:
            raise HTTPException(
    ...
    def get_comment_replies(self, comment_id: str, skip: int = 0, limit: int = 50) -> List[Comment]:
        """
    ...
    def delete_comment(self, comment_id: str, user_id: str) -> None:
        ...
        replies_count = self.db.query(Comment).filter(Comment.parent_id == comment_id).count()
        if replies_count > 0:
            raise HTTPException(
    ...
    def get_comment_statistics(self, celebrity_id: str) -> Dict[str, Any]:
        ...
        total_comments = self.db.query(Comment).filter(
            Comment.celebrity_id == celebrity_id
        ).count()
        ...
        active_commenters = self.db.query(
            Comment.user_id,
    ...
    def get_user_comment_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        recent_comments = self.db.query(Comment).filter(
            and_(
                Comment.user_id == user_id,
                Comment.created_at >= datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
            )
        ).count()
