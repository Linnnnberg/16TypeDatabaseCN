...
class JSONUploadService:
        ...
        try:
            ...
            for i, celeb in enumerate(upload_data.celebrities):
                if celeb.mbti not in valid_mbti_types:
                    validation_errors.append(f"Celebrity {i+1} ({celeb.name}): Invalid MBTI type '{celeb.mbti}'")
                
                # Check for duplicate names
                existing_celebrity = self.celebrity_service.get_celebrity_by_name(celeb.name)
                if existing_celebrity:
                    validation_errors.append(f"Celebrity {i+1} ({celeb.name}): Already exists in database")
    ...
    def process_upload_data(self, upload_data: UploadData) -> Dict[str, Any]:
        ...
        try:
            ...
            system_user = self.db.query(User).filter(User.role == UserRole.SYSTEM).first()
            if not system_user:
                return {
                    "success": False,
                    "errors": ["System user not found. Please run create_admin.py first."],
                    "imported_count": 0
                }
        ...
        for file_path in json_files:
            try:
                ...
                if process_result["success"]:
                    ...
                    # Create success log
                    success_log_path = self.processed_dir / f"{file_path.stem}_success.txt"
                    with open(success_log_path, 'w', encoding='utf-8') as f:
                        f.write(f"File: {file_path.name}\n")
                        f.write(f"Timestamp: {datetime.now().isoformat()}\n")
                        f.write(f"Imported: {process_result['imported_count']} celebrities\n")
                    ...
                    results["failed_imports"] += 1
                    results["errors"].extend([f"{file_path.name}: {error}" for error in process_result["errors"]])
