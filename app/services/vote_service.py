...
class VoteService:
    ...
    def create_vote(self, user_id: str, vote_data: VoteCreate) -> Vote:
        ...
        celebrity = self.db.query(Celebrity).filter(Celebrity.id == vote_data.celebrity_id).first()
        if not celebrity:
            raise HTTPException(
        ...
        existing_vote = self.db.query(Vote).filter(
            and_(
                ...
                Vote.celebrity_id == vote_data.celebrity_id
            )
        ).first()
        ...
        today = date.today()
        daily_stats = self.db.query(DailyUserStats).filter(
            and_(
    ...
    def get_celebrity_vote_statistics(self, celebrity_id: str) -> Dict[str, Any]:
        ...
        votes_with_reason = self.db.query(func.count(Vote.id)).filter(
            and_(
                Vote.celebrity_id == celebrity_id,
                Vote.reason.isnot(None)
            )
        ).scalar()
    ...
    def get_user_vote_statistics(self, user_id: str) -> Dict[str, Any]:
        ...
        # Get today's votes
        today = date.today()
        today_votes = self.db.query(func.count(Vote.id)).filter(
            and_(
                Vote.user_id == user_id,
