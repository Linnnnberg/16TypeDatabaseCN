...
class CelebrityService:
    ...
    def create_celebrity(self, celebrity_data: CelebrityCreate) -> Celebrity:
        ...
        existing_celebrity = self.db.query(Celebrity).filter(
            or_(
    ...
    def get_celebrity_by_name(self, name: str) -> Optional[Celebrity]:
        ...
        return self.db.query(Celebrity).filter(
            or_(
                Celebrity.name == name,
                Celebrity.name_en == name
            )
        ).first()
    ...
    def update_celebrity(self, celebrity_id: str, celebrity_data: CelebrityUpdate) -> Celebrity:
    ...
    def add_tag_to_celebrity(self, celebrity_id: str, tag_name: str) -> CelebrityTag:
        ...
        existing_tag = self.db.query(CelebrityTag).filter(
            CelebrityTag.celebrity_id == celebrity_id,
            CelebrityTag.tag_id == tag.id
        ).first()
    ...
    def remove_tag_from_celebrity(self, celebrity_id: str, tag_name: str) -> bool:
        ...
        celebrity_tag = self.db.query(CelebrityTag).filter(
            CelebrityTag.celebrity_id == celebrity_id,
            CelebrityTag.tag_id == tag.id
        ).first()
    ...
    def get_celebrities_by_tag(self, tag_name: str, skip: int = 0, limit: int = 100) -> List[Celebrity]:
        """Get celebrities by tag"""
        tag = self.db.query(Tag).filter(Tag.name == tag_name).first()
        if not tag:
            return []
        
        celebrities = self.db.query(Celebrity).join(CelebrityTag).filter(
            CelebrityTag.tag_id == tag.id
    ...
    def get_popular_celebrities(self, limit: int = 10) -> List[Celebrity]:
        ...
        celebrities = self.db.query(Celebrity, func.count(Celebrity.votes).label('vote_count')).outerjoin(
            Celebrity.votes
        ).group_by(
            Celebrity.id
        ).order_by(
            func.count(Celebrity.votes).desc()
        ).limit(limit).all()
