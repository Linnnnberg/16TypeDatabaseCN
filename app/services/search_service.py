...
class SearchService:
    ...
    def _hybrid_search(self, base_query, query: str, skip: int, limit: int) -> List[Dict[str, Any]]:
        """Hybrid search with relevance scoring"""
        ...
        search_query = base_query.add_columns(
            ...
            case(
                ...
                (func.lower(Celebrity.tags.any(Tag.name.contains(query_lower))), 40),
                # Default score
                else_=0
        ...
        for celebrity, relevance_score in results:
            ...
            result['match_type'] = self._determine_match_type(query_lower, celebrity, relevance_score)
            formatted_results.append(result)
        
        return formatted_results
    
    def _search_by_name(self, base_query, query: str, skip: int, limit: int) -> List[Dict[str, Any]]:
    ...
    def _search_by_description(self, base_query, query: str, skip: int, limit: int) -> List[Dict[str, Any]]:
    ...
    def _search_by_tag(self, base_query, query: str, skip: int, limit: int) -> List[Dict[str, Any]]:
        """Search by tag only"""
        query_lower = query.lower()
    ...
    def _search_by_mbti(self, base_query, query: str, skip: int, limit: int) -> List[Dict[str, Any]]:
        """Search by MBTI type votes"""
        query_upper = query.upper()
        
        # Get celebrities with votes for the specified MBTI type
        search_query = base_query.add_columns(
            func.count(Vote.id).label('vote_count_for_type')
        ).join(Vote, Celebrity.id == Vote.celebrity_id).filter(
            Vote.mbti_type == query_upper
        ).group_by(Celebrity.id).order_by(desc('vote_count_for_type'))
    ...
    def _apply_mbti_filter(self, query, mbti_type: str):
        ...
        return query.join(Vote, Celebrity.id == Vote.celebrity_id).filter(
            Vote.mbti_type == mbti_type.upper()
        ).group_by(Celebrity.id)
    
    def _apply_tag_filter(self, query, tag_name: str):
        """Apply tag filter"""
        return query.filter(
            Celebrity.tags.any(Tag.name.contains(tag_name))
        )
    ...
    def _apply_popularity_filter(self, query, popularity_filter: str):
        ...
        if popularity_filter == "popular":
            ...
            ).outerjoin(Vote, Celebrity.id == Vote.celebrity_id).group_by(Celebrity.id).order_by(
                desc('vote_count')
            )
    ...
    def _determine_match_type(self, query_lower: str, celebrity: Celebrity, relevance_score: float) -> str:
    ...
    def get_search_suggestions(self, query: str, limit: int = 10) -> List[str]:
        ...
        name_suggestions = self.db.query(Celebrity.name).filter(
            func.lower(Celebrity.name).contains(query_lower)
        ).limit(limit // 2).all()
        
        # Get English name suggestions
        name_en_suggestions = self.db.query(Celebrity.name_en).filter(
            func.lower(Celebrity.name_en).contains(query_lower)
        ).limit(limit // 2).all()
        
        # Get tag suggestions
        tag_suggestions = self.db.query(Tag.name).filter(
        ...
        for suggestion in name_suggestions + name_en_suggestions + tag_suggestions:
            if suggestion[0] and suggestion[0].lower() not in [s.lower() for s in all_suggestions]:
    ...
    def get_search_analytics(self) -> Dict[str, Any]:
        ...
        popular_tags = self.db.query(
            Tag.name,
            func.count(CelebrityTag.celebrity_id).label('usage_count')
        ).join(CelebrityTag, Tag.id == CelebrityTag.tag_id).group_by(Tag.id).order_by(
            desc('usage_count')
        ).limit(10).all()
        
        # Get popular MBTI types
        popular_mbti = self.db.query(
            Vote.mbti_type,
            func.count(Vote.id).label('vote_count')
        ).group_by(Vote.mbti_type).order_by(desc('vote_count')).all()
        ...
        return {
            ...
            "popular_mbti_types": [{"type": mbti_type, "count": count} for mbti_type, count in popular_mbti]
        } 