...
def test_endpoint(url, expected_status=200, timeout=10):
    ...
    try:
        ...
        if response.status_code == expected_status:
            ...
            print(f"FAILED: {url} - Expected: {expected_status}, Got: {response.status_code}")
            return False
...
def main():
    ...
    tests.append(("API Root", lambda: test_json_endpoint(f"{base_url}/api", ["message", "version", "status"])))
    tests.append(("Test Endpoint", lambda: test_json_endpoint(f"{base_url}/test", ["message", "mbti_types"])))
    tests.append(("Database Test", lambda: test_json_endpoint(f"{base_url}/db-test", ["status"])))
    tests.append(("Environment Info", lambda: test_json_endpoint(f"{base_url}/env", ["python_version", "fastapi_version"])))
    
    # Endpoint existence tests (accept various status codes)
    tests.append(("Auth Endpoint Exists", lambda: test_endpoint(f"{base_url}/auth/login", expected_status=[405, 404])))
    tests.append(("Celebrities Endpoint Exists", lambda: test_endpoint(f"{base_url}/api/celebrities", expected_status=[200, 401, 404])))
    
    # Documentation tests
    tests.append(("API Docs", lambda: test_endpoint(f"{base_url}/docs")))
    tests.append(("OpenAPI Schema", lambda: test_json_endpoint(f"{base_url}/openapi.json", ["openapi", "info", "paths"])))
