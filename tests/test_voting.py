    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if celebrities:
                ...
                test_config.add_test_result("Create Vote", False, "No celebrities found")
                return False
        else:
            test_config.add_test_result("Create Vote", False, "Failed to get celebrities")
            return False
    except Exception as e:
...
def test_get_celebrity_votes():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if celebrities:
                celebrity_id = celebrities[0]["id"]
                response = test_config.make_request("GET", f"/votes/celebrity/{celebrity_id}")
                success = response.status_code == 200
                data = response.json()
                test_config.add_test_result(
                    "Get Celebrity Votes",
                ...
                test_config.add_test_result("Get Celebrity Votes", False, "No celebrities found")
                return False
        else:
            test_config.add_test_result("Get Celebrity Votes", False, "Failed to get celebrities")
            return False
    except Exception as e:
...
def test_delete_vote():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            test_config.add_test_result("Delete Vote", False, "Failed to get user votes")
            return False
    except Exception as e:
        ...
        if response.status_code == 200:
            ...
            if celebrities:
                ...
                if response1.status_code == 201:
                    ...
                    response2 = test_config.make_request("POST", "/votes/", json=vote_data)
                    ...
                    test_config.add_test_result("Duplicate Vote Validation", False, "Failed to create first vote")
                    return False
            else:
                test_config.add_test_result("Duplicate Vote Validation", False, "No celebrities found")
                return False
        else:
            test_config.add_test_result("Duplicate Vote Validation", False, "Failed to get celebrities")
            return False
    except Exception as e:
