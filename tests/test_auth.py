...
def test_admin_login():
    ...
    try:
        response = requests.post(
            f"{test_config.base_url}/auth/login", json=test_config.admin_credentials
        )
        success = response.status_code == 200
        if success:
            data = response.json()
            ...
            test_config.add_test_result(
                "Admin Login", True, f"Token received: {test_config.auth_token[:20]}..."
            )
        else:
            test_config.add_test_result(
                "Admin Login",
...
def test_user_registration():
    ...
    try:
        ...
        response = requests.post(
            f"{test_config.base_url}/auth/signup", json=test_user_data
        )
...
def test_invalid_login():
    ...
    try:
        ...
        response = requests.post(
            f"{test_config.base_url}/auth/login", json=invalid_credentials
        )
        success = response.status_code == 401  # Should fail
        test_config.add_test_result(
            "Invalid Login", success, f"Status: {response.status_code}, Expected: 401"
        )
        return success
    except Exception as e:
